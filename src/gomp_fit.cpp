...
arma::colvec gompfun(const arma::colvec& x, const arma::colvec& par){
    return(par(2) * par(1) * exp(par(1) + par(2) * x - par(1) * exp(par(2) * x)));
}
...
Rcpp::List gomp_fit_c(const arma::colvec& x, const arma::colvec& y, const arma::colvec& parstart,
    ...
    obj(2) = std::inner_product(res.begin(), res.end(), res.begin(), 0.0);
    obj(1) = 2 * obj(2);
    arma::mat J, JTJ, lammat, g;
    arma::colvec dC, newpar, newres;
    double C, Cnew;

    while(obj(1) - obj(2) > eps){
        J = arma::join_rows(df_deta(x, par(1), par(2)), df_db(x, par(1), par(2)));
        JTJ = arma::cross(J, J);
        lamvec.fill(lambda);
